  ...
  EitherFailureOr<bool> authenticate({
    ...
    try {
      final result = await _localAuthentication.authenticate(
        ...
        authMessages:
            authMessages ?? [const AndroidAuthMessages(biometricHint: '')],
      ...
      if (error.code == notEnrolled) {
        return Left(Failure.generic(
            title: 'No hardware', error: error, stackTrace: stackTrace));
      } else if (error.code == lockedOut ||
          error.code == permanentlyLockedOut) {
        return Left(Failure.generic(
            title: 'Locked out', error: error, stackTrace: stackTrace));
      } else {
        return Left(Failure.generic(
            title: 'Local auth error', error: error, stackTrace: stackTrace));
      }
    }
  }
